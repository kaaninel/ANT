...
HF_CHECKPOINT  = "checkpoints/chat/checkpoint_best.pt"   # best from Colab run (11% QA at step 3000)
OUTPUT_DIR     = "checkpoints/overnight"
DEVICE         = "mps"   # M4 MacBook Air

# Data
N_WIKI   = 200_000   # 4× more Wikipedia (overnight can afford it)
N_SHELL  = 10_000
N_QA     = 50_000
N_CHAT   = 10_000

# Training — designed for ~8h on M4 at ~0.7 it/s
# Phase 1 (LM): fix language quality after stride=4 Colab run
# Phase 2 (LM+QA): rebuild QA from 11% → 60%+
# Phase 3 (all): refine everything together
PHASE1_STEPS = 5_000    # ~2h  — LM repair pass
PHASE2_STEPS = 10_000   # ~4h  — memory QA
PHASE3_STEPS = 3_000    # ~1.5h — final refinement
# Total: ~18,000 steps ≈ 7.5h
...
LR = 2e-4  # slightly lower than fresh (3e-4) since we're continuing
...
def main():
    ...
    if not os.path.exists(local_ckpt):
        ok = download_checkpoint(HF_REPO, HF_CHECKPOINT, local_ckpt)
        if not ok:
            # Fall back to any local checkpoint
            fallbacks = [
                "checkpoints/chat/checkpoint_best.pt",
                "checkpoints/multitask/checkpoint_best.pt",
                "checkpoints/chat/checkpoint_latest.pt",
            ]
            for fb in fallbacks:
                if os.path.exists(fb):
                    print(f"  Using local fallback: {fb}")
                    local_ckpt = fb
                    break
            else:
